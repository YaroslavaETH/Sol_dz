...
/**
 * @title Интерфейс для MultiSigWallet
 * @dev Используется для проверки владельцев мультисиг кошелька
 */
    ...
    OwnableUpgradeable
{
    ...
    /// @notice Модификатор, проверяющий что вызывающий является одним из владельцев MultiSigWallet
    modifier onlyMultiSigOwner() {
        if (!IMultiSigWallet(owner()).isOwner(msg.sender)) revert NotMultiSigOwner();
        _;
    }
    ...
    /// @notice Ошибка, вызываемая когда вызывающий не является владельцем MultiSigWallet
    ...
    ) external virtual onlyMultiSigOwner {
        if (evacuating) {
     ...
     */
    function confirmWithdrawal(uint256 operationId) external virtual onlyOwner {
        // Проверяем, что такая операция существует в списке неподтвержденных.
        uint256 indexToRemove = unconfirmedOperationIndex[operationId];
